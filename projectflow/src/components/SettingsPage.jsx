    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, backdropFilter: "blur(4px)" }}>
      <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 22, width: "100%", maxWidth: 870, height: "88vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 24px 80px rgba(15,23,42,0.18)" }}>
        ...
        <div style={{ padding: "16px 22px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", gap: 12, background: "#ffffff" }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: "linear-gradient(135deg,#4f46e5,#7c3aed)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>⚙️</div>
          <div>
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          ...
          <div style={{ width: 185, borderRight: "1px solid #f1f5f9", padding: "14px 10px", display: "flex", flexDirection: "column", gap: 3, background: "#fafbfc" }}>
            {tabs.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{ background: tab === t.id ? "rgba(79,70,229,0.08)" : "transparent", border: tab === t.id ? "1px solid rgba(79,70,229,0.2)" : "1px solid transparent", borderRadius: 9, padding: "9px 12px", textAlign: "left", color: tab === t.id ? "#4f46e5" : "#64748b", fontSize: 13, fontWeight: tab === t.id ? 600 : 400, display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                {t.icon} {t.label}
              </button>
            ))}
            ...
            {tab === "users" && (
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
                  ...
                  {canManage && <button onClick={() => setShowAdd(true)} style={{ background: "linear-gradient(135deg,#4f46e5,#7c3aed)", border: "none", borderRadius: 8, padding: "7px 13px", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>+ 사용자 추가</button>}
                </div>
                ...
                <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 14, overflow: "hidden" }}>
                  ...
                  {filtered.map((user, i) => {
                    ...
                    return (
                      <div key={user.id} style={{ display: "grid", gridTemplateColumns: "2fr 2.2fr 1fr 1.1fr 1fr 100px", alignItems: "center", padding: "11px 16px", borderBottom: i < filtered.length - 1 ? "1px solid #f8fafc" : "none", background: isMe ? "rgba(79,70,229,0.03)" : "transparent" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <Avatar name={user.name} size={30} color={role.color} />
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 500, color: "#1e293b" }}>{user.name}{isMe && <span style={{ fontSize: 10, color: "#4f46e5", marginLeft: 5 }}>(나)</span>}</div>
                        ...
                        <div>
                          {canManage && !isMe
                            ? <button onClick={() => doToggle(user.id)} style={{ background: user.status === "활성" ? "rgba(5,150,105,0.08)" : "rgba(100,116,139,0.08)", border: user.status === "활성" ? "1px solid rgba(5,150,105,0.3)" : "1px solid #e2e8f0", borderRadius: 8, padding: "3px 9px", color: user.status === "활성" ? "#059669" : "#94a3b8", fontSize: 11, cursor: "pointer" }}>{user.status === "활성" ? "● 활성" : "○ 비활성"}</button>
                            : <span style={{ fontSize: 11, color: user.status === "활성" ? "#059669" : "#94a3b8" }}>{user.status === "활성" ? "● 활성" : "○ 비활성"}</span>}
                        </div>
                        <div style={{ display: "flex", gap: 5, justifyContent: "flex-end" }}>
                          {canManage && <>
                            <button onClick={() => setEdit({ ...user })} onMouseEnter={e => e.currentTarget.style.background = "rgba(79,70,229,0.08)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"} style={{ background: "transparent", border: "1px solid #e2e8f0", borderRadius: 6, padding: "4px 8px", color: "#64748b", fontSize: 11, cursor: "pointer" }}>수정</button>
                            {!isMe && <button onClick={() => doDel(user.id)} onMouseEnter={e => { e.currentTarget.style.background = "rgba(220,38,38,0.08)"; e.currentTarget.style.color = "#dc2626"; }} onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#64748b"; }} style={{ background: "transparent", border: "1px solid #e2e8f0", borderRadius: 6, padding: "4px 8px", color: "#64748b", fontSize: 11, cursor: "pointer", transition: "all 0.2s" }}>삭제</button>}
                          </>}
                        </div>
                      </div>
                    );
                    ...
                    <div key={key} style={{ background: "#fafbfc", border: `1px solid ${role.color}25`, borderRadius: 14, padding: "14px 18px", display: "flex", alignItems: "flex-start", gap: 14 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                          ...
                          {currentUser.role === key && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 10, background: "rgba(79,70,229,0.1)", color: "#4f46e5" }}>내 권한</span>}
                        </div>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                          {Object.entries(PERMISSIONS[key]).map(([perm, ok]) => {
                            ...
                            return <span key={perm} style={{ fontSize: 11, padding: "2px 9px", borderRadius: 8, background: ok ? "rgba(5,150,105,0.08)" : "rgba(100,116,139,0.06)", color: ok ? "#059669" : "#94a3b8", border: ok ? "1px solid rgba(5,150,105,0.2)" : "1px solid #e2e8f0" }}>{ok ? "✓" : "✗"} {labels[perm]}</span>;
                          })}
                        </div>
                      </div>
                    </div>
                ...
                <div style={{ background: "#fafbfc", border: "1px solid #e2e8f0", borderRadius: 14, padding: "16px 18px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                    {data.users.map(u => {
                      ...
                      return (
                        <button key={u.id} onClick={() => !isActive && setData(d => ({ ...d, currentUserId: u.id }))} style={{ background: isActive ? "rgba(79,70,229,0.06)" : "transparent", border: isActive ? "1px solid rgba(79,70,229,0.2)" : "1px solid transparent", borderRadius: 10, padding: "8px 12px", display: "flex", alignItems: "center", gap: 10, cursor: isActive ? "default" : "pointer", textAlign: "left" }}>
                          <Avatar name={u.name} size={26} color={r.color} />
                          <div style={{ flex: 1 }}>
                            <span style={{ fontSize: 12, fontWeight: 500, color: isActive ? "#4f46e5" : "#475569" }}>{u.name}</span>
                            {isActive && <span style={{ fontSize: 10, color: "#4f46e5", marginLeft: 6 }}>현재</span>}
                          </div>
