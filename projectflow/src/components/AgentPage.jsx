...
function TaskRow({ task, team }) {
  const sc = task.status === "완료" ? "#059669" : task.status === "진행중" ? "#4f46e5" : "#94a3b8";
  return (
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        <StatCard label="전체 업무" value={stats.total}      color="#4f46e5" icon="📋" />
        <StatCard label="완료"      value={stats.done}       color="#059669" icon="✅" />
        <StatCard label="진행중"    value={stats.inProgress} color="#7c3aed" icon="⚡" />
        <StatCard label="대기"      value={stats.waiting}    color="#d97706" icon="⏳" />
      </div>
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b", marginBottom: 4 }}>
          <span>전체 완료율</span><span style={{ fontWeight: 700, color: "#059669" }}>{stats.doneRate}%</span>
        </div>
        <ProgBar value={stats.doneRate} color="#059669" height={8} />
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#64748b", marginTop: 8, marginBottom: 4 }}>
          <span>평균 진행률</span><span style={{ fontWeight: 700, color: "#4f46e5" }}>{stats.avgProgress}%</span>
        </div>
        <ProgBar value={stats.avgProgress} color="#4f46e5" height={8} />
      </div>
      {overdue.length > 0 && (
        <div style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#dc2626" }}>
          ⚠️ 마감 초과 업무 {overdue.length}건이 있습니다. 즉각적인 확인이 필요합니다.
        </div>
      )}
      {overdue.length === 0 && (
        <div style={{ background: "rgba(5,150,105,0.06)", border: "1px solid rgba(5,150,105,0.2)", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#059669" }}>
        ...
        {teamStats.map(({ team, tt, td, ti, tw, avg }) => (
          <div key={team.id} style={{ background: `${team.color}06`, border: `1px solid ${team.color}20`, borderRadius: 10, padding: "10px 14px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: avg >= 70 ? "#059669" : avg >= 40 ? "#4f46e5" : "#d97706" }}>{avg}%</span>
            </div>
        ...
        : [...wipList].sort((a, b) => b.progress - a.progress).map(t => (
            <div key={t.id}>
              ...
              <div style={{ paddingLeft: 14, marginBottom: 4 }}>
                <ProgBar value={t.progress} color={getTeam(t.teamId)?.color || "#4f46e5"} />
              </div>
            </div>
          ))
      }
  ...
  if (overdue.length === 0) {
    return (
      <div>
        ...
        <div style={{ background: "rgba(5,150,105,0.06)", border: "1px solid rgba(5,150,105,0.2)", borderRadius: 10, padding: "14px", textAlign: "center", fontSize: 13, color: "#059669" }}>
  ...
  return (
    <div>
      ...
      {overdue.map(t => {
        ...
        return (
          <div key={t.id} style={{ background: "rgba(239,68,68,0.04)", border: "1px solid rgba(239,68,68,0.15)", borderRadius: 8, padding: "8px 12px", marginBottom: 6 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 11, background: "#fef2f2", color: "#dc2626", padding: "1px 7px", borderRadius: 6, fontWeight: 600 }}>+{daysOver}일 초과</span>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        <StatCard label="완료"   value={done}               color="#059669" icon="✅" />
        <StatCard label="진행중" value={wip}                color="#4f46e5" icon="⚡" />
      ...
      {tasks.filter(t => (t.subtasks||[]).length > 0).map(t => {
        ...
        return (
          <div key={t.id} style={{ marginBottom: 8, border: "1px solid #f1f5f9", borderRadius: 8, overflow: "hidden" }}>
            ...
            {subs.map(s => {
              const sc = s.status === "완료" ? "#059669" : s.status === "진행중" ? "#4f46e5" : "#94a3b8";
              return (
...
function MonthlyView({ stats, teamStats, overdue }) {
  ...
  const gradeColor = { A: "#059669", B: "#4f46e5", C: "#d97706", D: "#dc2626" }[grade];
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        <StatCard label="완료 업무" value={stats.done}       color="#059669" icon="✅" />
        <StatCard label="진행중"    value={stats.inProgress} color="#4f46e5" icon="⚡" />
        <StatCard label="지연"      value={overdue.length}   color="#dc2626" icon="⚠️" />
        <StatCard label="완료율"    value={`${stats.doneRate}%`} color="#7c3aed" icon="📈" />
      </div>
      {topTeam && (
        <div style={{ background: "rgba(79,70,229,0.06)", border: "1px solid rgba(79,70,229,0.15)", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
...
function generatePDF(tasks, teams) {
  ...
  const teamRows = teams.map(team => {
    ...
    return `
      <tr>
        ...
        <td style="text-align:center;color:#059669;font-weight:bold">${td}</td>
        <td style="text-align:center;color:#4f46e5">${tt.filter(t=>t.status==="진행중").length}</td>
  ...
  const taskRows = tasks.map(t => {
    const team = getTeam(t.teamId);
    const sc   = t.status === "완료" ? "#059669" : t.status === "진행중" ? "#4f46e5" : "#94a3b8";
    return `
      <tr>
        <td>${t.title}</td>
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .header { display:flex; align-items:center; justify-content:space-between; border-bottom:3px solid #4f46e5; padding-bottom:16px; margin-bottom:24px; }
    .logo { font-size:22px; font-weight:900; color:#4f46e5; }
    .meta { font-size:11px; color:#94a3b8; text-align:right; }
    h2 { font-size:15px; font-weight:700; color:#1e293b; margin:24px 0 12px; padding-left:10px; border-left:4px solid #4f46e5; }
    ...
    th { background:#4f46e5; color:#fff; padding:8px 10px; font-size:11px; text-align:left; }
    td { padding:7px 10px; border-bottom:1px solid #f1f5f9; font-size:12px; }
    tr:nth-child(even) td { background:#fafbfc; }
    .alert { background:#fef2f2; border:1px solid #fecaca; border-radius:8px; padding:10px 14px; color:#dc2626; font-size:12px; margin-bottom:16px; }
    .ok { background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px; padding:10px 14px; color:#059669; font-size:12px; margin-bottom:16px; }
...
<body>
  ...
  <h2>📊 프로젝트 종합 현황</h2>
  <div class="stat-grid">
    <div class="stat-card"><div class="stat-val" style="color:#4f46e5">${stats.total}</div><div class="stat-lbl">전체 업무</div></div>
    <div class="stat-card"><div class="stat-val" style="color:#059669">${stats.done}</div><div class="stat-lbl">완료</div></div>
    <div class="stat-card"><div class="stat-val" style="color:#7c3aed">${stats.inProgress}</div><div class="stat-lbl">진행중</div></div>
    <div class="stat-card"><div class="stat-val" style="color:#d97706">${stats.waiting}</div><div class="stat-lbl">대기</div></div>
    <div class="stat-card"><div class="stat-val" style="color:#dc2626">${overdue.length}</div><div class="stat-lbl">마감 초과</div></div>
  </div>
  <div class="prog-wrap">
    <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:4px"><span>전체 완료율</span><span style="color:#059669;font-weight:bold">${doneRate}%</span></div>
    <div class="prog-bar-bg"><div class="prog-bar" style="width:${doneRate}%;background:#059669"></div></div>
  </div>
  <div class="prog-wrap">
    <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:4px"><span>평균 진행률</span><span style="color:#4f46e5;font-weight:bold">${stats.avgProgress}%</span></div>
    <div class="prog-bar-bg"><div class="prog-bar" style="width:${stats.avgProgress}%;background:#4f46e5"></div></div>
  </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {QUICK_PROMPTS.map(p => (
                <button
                  ...
                  onClick={() => send(p.text)}
                  style={{
                    background: "rgba(79,70,229,0.06)", border: "1px solid rgba(79,70,229,0.2)", borderRadius: 20,
                    padding: "5px 13px", fontSize: 12, color: "#4f46e5", cursor: "pointer",
                  }}
                >{p.label}</button>
              ))}
            </div>
